    ...
    async def handle_voice_join(self, member: discord.Member):
        ...
        try:
            ...
            if new_nickname:
                ...
                self.logger.info(f"Changed {member.display_name}'s nickname to '{new_nickname}' in {after.channel.name if after.channel else 'unknown channel'}")
...
async def restore_command(interaction: discord.Interaction, member: Optional[discord.Member] = None):
    ...
    try:
        ...
        if hasattr(target_member, 'guild'):
...
async def find_nickname_command(interaction: discord.Interaction, search_term: str):
    ...
    try:
        ...
        nicknames = bot.db.get_guild_nicknames(interaction.guild.id)
        matches = [nick for nick in nicknames if search_term.lower() in nick.nickname.lower()]
        ...
        if matches:
            ...
            for match in matches[:10]:  # Limit to 10 results
                results.append(f"• {match.nickname}")
            
            embed.add_field(
                name=f"Found {len(matches)} matches",
                value="\n".join(results),
                inline=False
            )
            ...
            await interaction.response.send_message(f"❌ No nicknames found matching '{search_term}'.", ephemeral=True)
    
    except Exception as e:
        bot.logger.error(f"Error searching nicknames: {e}")
        await interaction.response.send_message("❌ Error searching nicknames.", ephemeral=True)
...
async def setup_commands(bot):
    ...
    bot.tree.add_command(restore_command)
    bot.tree.add_command(add_nickname_command)
    bot.tree.add_command(remove_nickname_command)
    bot.tree.add_command(list_nicknames_command)
    bot.tree.add_command(find_nickname_command)
    bot.tree.add_command(bot_health_command)